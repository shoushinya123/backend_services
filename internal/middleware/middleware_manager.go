...
func (m *MiddlewareManager) CheckHealth() (map[string]HealthStatus, error) {
	...
	// 检查Milvus
	if m.milvus != nil && m.milvus.Ready() {
		health["milvus"] = HealthStatus{
			Status:    "healthy",
			Timestamp: time.Now(),
		}
	} else {
...
// GetMilvus 获取Milvus服务
func (m *MiddlewareManager) GetMilvus() *MilvusService {
	return m.milvus
}
