...
func NewKnowledgeService() *KnowledgeService {
	...
	case "qdrant":
		vectorSize := cfg.Knowledge.VectorStore.Qdrant.VectorSize
		if vectorSize == 0 && embedder != nil && embedder.Dimensions() > 0 {
			vectorSize = embedder.Dimensions()
		}
		qdrantStore, err := knowledge.NewQdrantVectorStore(knowledge.QdrantOptions{
			Endpoint:         fmt.Sprintf("%s:%d", cfg.Knowledge.VectorStore.Qdrant.Host, cfg.Knowledge.VectorStore.Qdrant.Port),
			APIKey:           cfg.Knowledge.VectorStore.Qdrant.APIKey,
			CollectionPrefix: cfg.Knowledge.VectorStore.Qdrant.CollectionPrefix,
			VectorSize:       vectorSize,
			Distance:         cfg.Knowledge.VectorStore.Qdrant.Distance,
			UseTLS:           cfg.Knowledge.VectorStore.Qdrant.UseTLS,
			Timeout:          15 * time.Second,
		})
		if err != nil {
			log.Printf("[knowledge] init qdrant vector store failed, fallback to database: %v", err)
			vectorStore = knowledge.NewDatabaseVectorStore(database.DB)
		} else {
